---
layout: default
title: Experience
description: Selected research, instrumentation, calibration, radiation-safety and data-analysis experience.
permalink: /experience/
---
<h1>Experience</h1>
<p class="page-intro">Work across scientific research, detector systems, calibration, radiation protection, data processing, and international collaboration.</p>
{% include experiences.html %}
<section class="section">
  <h2 class="section-title"><span class="fa-stack fa-xs"><i class="fas fa-circle fa-stack-2x"></i><i class="fas fa-users fa-stack-1x fa-inverse"></i></span>Collaboration & Leadership</h2>
  <div class="card-grid">
    <article class="content-card"><h3>International collaboration</h3><p>Research within the Pierre Auger Collaboration, contributing to technical reviews, workshops, detector operations, and shared analysis workflows.</p></article>
    <article class="content-card"><h3>Teaching and mentoring</h3><p>Supported student learning in machine learning for particle physics and astrophysics, and supervised students during a field visit to the Boulby Underground Laboratory.</p></article>
    <article class="content-card"><h3>Field operations</h3><p>Experience with radio detector stations, fluorescence telescopes, LiDAR systems, calibration campaigns, technical shifts, and safety-focused procedures.</p></article>
    <article class="content-card"><h3>Technical communication</h3><p>Prepared research presentations, analysis notes, diagnostic dashboards, documentation, and explanations for specialist and non-specialist audiences.</p></article>
  </div>
</section>
